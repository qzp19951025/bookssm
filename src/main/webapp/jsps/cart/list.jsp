		...
		<c:otherwise>
			<table width="95%" align="center" cellpadding="0" cellspacing="0">
				...
				<c:forEach items="${clist}" var="c">
					<tr align="center">
						<td align="left"><input value="${c.cartItemId }"
							type="checkbox" name="checkboxBtn" checked="checked" /></td>
						<td align="left" width="70px"><a class="linkImage"
							href="<c:url value='/bookAction?flag=getBookByBid&bid=${c.book.bid}'/>"><img
								border="0" width="54" align="top"
								src="<c:url value='/${c.book.image_b }'/>" /></a></td>
						<td align="left" width="400px"><a
							href="<c:url value='/bookAction?flag=getBookByBid&bid=${c.book.bid}'/>"><span>${c.book.bname }</span></a>
						</td>
						<td><span>&yen;<span class="currPrice"
								id="${c.cartItemId}CurrPrice">${c.book.currPrice }</span></span></td>
						<td><a class="jian" id="${c.cartItemId }Jian"></a><input
							class="quantity" readonly="readonly"
							id="${c.cartItemId }Quantity" type="text" value="${c.quantity }" /><a
							class="jia" id="${c.cartItemId }Jia"></a></td>
						<td width="100px"><span class="price_n">&yen;<span
								class="subTotal" id="${c.cartItemId }Subtotal">${c.subtotal }</span></span>
						</td>
						<td><a
							href="<c:url value='/cartItemAction?flag=removeCartItemByCid&cartItemId=${c.cartItemId }'/>">删除</a></td>
					</tr>
				</c:forEach>
				<tr>
					<td colspan="4" class="tdBatchDelete"><a id="batchRemove">批量删除</a></td>
			...
			<script type="text/javascript">
									...
									function() {
										...
										if (quantity == 1) {
											if (confirm("您是否真要删除该条目？")) {
												...
												$
														.post(
																"/goods/cartItemAction?flag=removeCartItemByCidAjax",
																{
											...
											$
													.post(
															"/goods/cartItemAction?flag=minusQuantityByCidAjax",
															{
																"cartItemId" : cartItemId
															},
															function() {
					...
					$(".jia")
							.click(
									function() {
										...
										$
												.post(
														"/goods/cartItemAction?flag=plusQuantityByCidAjax",
														{
															"cartItemId" : cartItemId
														}, function() {
															window.location
							...
							.click(
									function() {
										if (confirm("确定要删除所选项目？")) {
											var cids = [];
											$(
													":checkbox[name=checkboxBtn][checked=true]")
													.each(
															function() {
																cids.push($(
																		this)
											...
											$
													.post(
															"/goods/cartItemAction?flag=removeCartItemsByCids",
															{
																"a[]" : cids
															},
															function() {
				...
				function jiesuan() {
					...
					$(":checkbox[name=checkboxBtn][checked=true]").each(
							function() {
								cids.push($(this).val());//把复选框的值添加到数组中
							});
					// 2. 把数组的值toString()，然后赋给表单的cartItemIds这个hidden
					$("#cartItemIds").val(cids.toString());
					// 把总计的值，也保存到表单中
